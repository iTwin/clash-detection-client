...
const clientLib = path.resolve(__dirname, "../../../lib/cjs");

function createConfig(shouldInstrument) {
  const config = {
    mode: "development",
    entry: glob.sync(path.resolve(clientLib, "test/**/*.test.js")),
    output: {
      path: path.resolve(clientLib, "test/webpack/"),
  ...
  if (shouldInstrument) {
    ...
    config.module.rules.push({
      test: /\.(jsx?|tsx?)$/,
      include: clientLib,
      exclude: path.join(clientLib, "test"),
